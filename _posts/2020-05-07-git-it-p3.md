...
After this, the text editor wil pop-up and will ask you to write the merge commit message. Enter the message then save and exit the text editor.
...
Bamm! Now you know the basics of branching and merging. Now it's time to learn one more important technique, rebasing. But before that let us discuss something what we call as `merge conflicts`.
...
Rebsaing helps in avoiding merge conflicts. So let's learn how rebasing is done in git.
...
Rebase is a better technique than merging because it doesnt gives you a merge commit, just the commits required.

As given by [Atlassian](https://www.atlassian.com/git/tutorials/rewriting-history/git-rebase), rebase can be visualized as follows:
...
It is possible that you might need to do this action multiple times, until every merge conflict is resolved. In case you mess up things, you can always run the `abort` command to start over:

```python
git rebase --abort
```

Huraay! You now know rebasing as well (You're a pro, I'm telling you already!). Now there's one last technique that we need to learn, which is used a lot when contributing to open source projects, squashing.
...
5. After the rebase is done, text editor will pop-up again, now you can write the commit message for the new commit `C7`. The commits are now squashed.
